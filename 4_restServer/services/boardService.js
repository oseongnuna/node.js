...
// 게시글 단건 수정: modifyInfo, 필요값 (title, content, author)
const updateInfo = async (bId, boardInfo) => {
  let { title, content, author } = boardInfo;
  let result = await mysql.query("modifyInfo", [title, content, author, bId]);
  return result;
};

// 게시글 단건 삭제: deleteBoard, 필수값(숫자, 1개) => board_id
const deleteBoard = async (bId) => {
  let info = await mysql.query("delInfo", bId);
  return info;
};

module.exports = { findAll, findByBoardId, addInfo, updateInfo, deleteBoard };
